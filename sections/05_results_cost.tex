\subsection{A few hundred ligands recover the map, and five modes carry its experimental agreement}
\label{sec:cost}
...
diagonal improves rank agreement with the experimental target-pair map on all four
kinase panels, by 0.033 on PKIS2 up to 0.058 on KiRHub, and improves ROC-AUC on
all four, by 0.014 on PKIS1 up to 0.063 on KiRHub (Figure~\ref{fig:cost}b).
...
outside the five-mode core raises rank agreement and ROC-AUC instead of lowering
them. Participation ratio counts directions, not their usefulness for a particular endpoint;
the spectrum and transfer test therefore answer different questions.
This is a within-scorer statement, and we do not extend it to comparisons between
scoring functions.

We release the audit as a command-line tool.
\texttt{analysis/target\_map\_audit.py} takes any dense ligand-by-target score
