...
18{,}000 cells. Pose selection is therefore common to all eight, so nothing that separates them can
be attributed to a different selected pose. The remaining differences are scorer- and
implementation-specific at fixed geometry.
...
A 44-column matrix, in other words, has fewer than two participation-ratio effective directions.
...
bound. Conditioning on coarse ligand size therefore reproduces part of the concentration, but not
most of it:
...
0.994. These controls show the disagreement tracks a chemical-domain shift strongly aligned with
molecular size rather than ordinary finite-support variation, but
