...
On the primary de-overlapped PDSP graph, which includes only target pairs with at least ten shared
...
reaches the sequence-identity baseline on ROC-AUC (0.710 versus 0.718) but remains below it on average
...
five leading modes carry half of the residual trace and improve rank agreement and ROC-AUC on all
four kinase panels. Average precision improves on only two, and the cross-panel interpretation is
post hoc. We did not tune over neighbouring values of $k$, and do not claim that five modes are
...
sensitivity rather than a substitute for uniformly matched structures. An exploratory pocket-volume
association is reported in Supplementary Section S2.4 and Supplementary Figure~S1. It was selected
after descriptor inspection, and the released pocket table has no executable pocket-definition
workflow; it is hypothesis-generating and does not establish a mechanism.
...
the two large raw Vina matrices. Removing it leaves structured geometry that is inexpensive to estimate,
but
that geometry belongs to a declared receptor panel, scoring pipeline and ligand library. External
...
rule. The practical recommendation is simple: remove the per-ligand offset, test the residual against
matched nulls, and estimate it on a pilot sample drawn from the intended chemical domain.
