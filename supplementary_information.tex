...
\title{Supplementary information\\[4pt]\large Spectral concentration in two large Vina
docking-score matrices: two-way centering reveals correlated residual variation without a
resolved target-ranking gain}
...
\begin{figure}[htbp]
\centering
\includegraphics{figures/figS7_residual_correlations.pdf}
\caption{\textbf{Residual target-correlation structure.}
\textbf{a,b}, Two-way-centered residual target correlations ordered by average-linkage
clustering of $1-r$. A subset of target labels is printed for legibility; the complete order
and family mapping are supplied in \texttt{results/residual\_target\_order.csv}.
\textbf{c,d}, Distributions of all off-diagonal correlations before and after centering.
Annotations give mean absolute correlation and the within-family minus between-family mean
absolute-correlation difference with a one-sided 5,000-label-permutation probability. The
broad-family associations are weak and do not establish biological specificity.}
\label{fig:s7}
\end{figure}
...
is a one-to-one re-expression of mean-squared inter-target correlation and adds no
independent statistical information to it. Its scale is interpretable as effective
components; mean absolute correlation, signed distributions, heatmaps and the full spectrum
retain information that PR discards.
\endgroup

\paragraph{Complete-case chemical support.}
Only 1.466\% of Docking-44 cells were missing, but they occurred in 3,354 ligands. Target
4mqs accounted for 3,352 missing cells, and the median incomplete row lacked one target.
Incomplete ligands had median molecular weight 399 Da and 28 heavy atoms, compared with 249
Da and 17 heavy atoms among complete cases. Standardized mean differences were 1.86 and
2.03. Thus the complete-case PR of 1.320 describes a chemically shifted support; full
descriptor results are in \path{results/complete_case_chemical_support.csv}.
...
\paragraph{Empirical residual-permutation null.}
Independently permuting every observed residual target column sampled exactly from each
target's non-Gaussian empirical residual values, after which transformation-matched two-way
centering and standardization were repeated. The latter step can slightly modify the
marginals. Median residual PR dimensions were 42.40
for Docking-44 and 56.48 for DOCKSTRING-58; both observed values were below all 500
realizations ($p=0.002$). Across 200 supports containing one ligand from each of 8,150
Docking-44 Butina clusters, observed residual PR had median 9.43 and the matched empirical
null median 42.33. The empirical null does not retain ligand-specific residual scale, but the
cluster-representative result shows that ordinary chemical redundancy does not explain the
separation.

\paragraph{Row-norm-preserving residual null.}
Independent Gaussian directions were projected into the target-zero-sum subspace and then
rescaled so that each simulated row had exactly the observed residual Euclidean norm. Median
PR dimensions were 42.63 and 56.62; the observed 9.27 and 18.19 on the matched supports fell
below all 500 realizations ($p=0.002$). The null retains ligand-specific residual scale but
not target-specific marginals, complementing rather than replacing the empirical column-
permutation null.

\paragraph{Residual modes and target families.}
Mean absolute target correlation fell from 0.694 to 0.245 in Docking-44 and from 0.624 to
0.157 in DOCKSTRING on the fixed characterization supports. Within-family values exceeded
between-family values by 0.030 and 0.017, but neither broad-family label permutation was
resolved at 0.05 ($p=0.073$ and 0.067). Residual PC1 retained strong correlations with ligand
size (absolute Spearman $|\rho|=0.72$ with molecular weight in Docking-44 and 0.77 with
heavy-atom count in DOCKSTRING; component signs are arbitrary). The residual covariance
therefore has interpretable target-specific physicochemical structure without establishing
a protein-family or specificity mechanism.
...
Before target-specific scaling, target-centered and two-way-centered scores differed by a
constant within ligand and consequently produced exactly identical target rankings and
accuracy 0.556. The fully residual-scaled score reached 0.564, so its operational difference
is attributable to target-specific scale and its interaction with the ligand offset.

Both Murcko- and Butina-cluster intervals included zero for residual minus absolute and
residual minus column-standardized scores. Conservative unions of the cluster-bootstrap 90\%
intervals were $-0.056$--0.051 and $-0.015$--0.049, respectively. Neither comparison met a
post hoc $\pm0.02$ equivalence margin; residual minus absolute also failed $\pm0.05$.
Holding the broad docking support fixed but comparing only within identical endpoint type
retained 132 evaluable ligands and 4,330 endpoint-specific pair instances; absolute,
column-standardized and residual accuracies were 0.553, 0.538 and 0.555. On human binding
Ki/Kd measurements, 107 ligands supplied 2,224 pairs and the same accuracies were 0.565,
0.554 and 0.579. The residual-minus-absolute cluster intervals were $-0.060$--0.064 and
$-0.051$--0.085. These sensitivities remain observational and precision limited:
target-pair co-observation had median support three, equivalence is not established, and no
result demonstrates retrieval on unmeasured targets.

Ordinary identity-shuffle probabilities for absolute and residual Vina were 0.014 and 0.024,
but median probabilities across 25 one-ligand-per-Murcko-cluster supports were 0.134 and
0.284. Leave-one-target-out residual-minus-absolute differences ranged from $-0.030$ to
0.010 in the broad benchmark and from $-0.027$ to 0.031 in human binding Ki/Kd. Finally,
collapsing repeated endpoint strata to 2,460 unique ligand--target-pair instances gave
absolute, column-standardized and residual accuracies 0.567, 0.541 and 0.562. These controls
reinforce the conclusion that neither score association nor representation differences are
precisely determined on the available support.
