...
\title{Supplementary information\\[4pt]\large Vina docking-score matrices are strongly
spectrally concentrated before, but not after, two-way centering}
...
\begin{figure}[htbp]
\centering
\includegraphics{figures/figS3_dti_all20.pdf}
...
Numbers identify arms in Table~S5; blue denotes arms above the outcome-related chance
...
All 20 scorer arms are the primary exploratory panel in Table~\ref{tab:s5all20} and
Fig.~\ref{fig:s3}. The 12 arms in Table~\ref{tab:s6dti} share data, architectures and
representations and were retained only as an explicitly outcome-restricted sensitivity.
Table~\ref{tab:s7selection} compares both panels. The positive column-standardized-PR
...
column and then applied the corresponding transformation. The counts above the
rank-wise 95th percentile null were 3 versus 7 for Docking-44 and 4 versus 10 for
DOCKSTRING-58. Each result was unchanged in five independent 100-permutation series. Because
centering changes the estimand and creates one exact linear
constraint, each surface is compared with its own transformed null.
