...
9.30 and 18.21. Chemical-cluster resampling and independently seeded chemical supports
confirmed the increase, while fitted additive-noise nulls showed that the residual surfaces
remained concentrated. On a dense matched
...
($-0.07$--0.92) afterward. We then expanded the operational comparison to 137 ligands, 38
targets and 2,522 non-tied observed experimental target pairs without activity imputation.
Absolute, column-standardized and two-way-residual Vina reached mean per-ligand pairwise
accuracies of 0.566, 0.546 and 0.564, with no resolved paired difference. An external
experimental target prior reached 0.525; a non-deployable prior fitted with each ligand's
scaffold cluster held out reached 0.592, revealing observation-cohort structure. Rankings
changed when more extensively profiled ligands were required. Two-way centering is therefore
useful for diagnosing residual score structure, but higher residual dimension is not by
itself a validated correction for target ranking.
...
target-panel composition from transformation effects. A dense matched spectral control and
an expanded observed-pair experimental benchmark show both what two-way centering
reveals---higher-dimensional residual structure---and what it does not establish: improved
target ranking.
\end{abstract}
...
score ties received half credit, and its 95\% interval resampled 95 Bemis--Murcko scaffold
clusters. All docking offsets, scales and imputation means were fitted after excluding the
...
(scaffold-bootstrap interval 0.502--0.623; Fig.~\ref{fig:preference}a). Two-way-residual and
column-standardized scores reached 0.564 (0.500--0.626) and 0.546 (0.483--0.603). The paired
residual-minus-column difference was 0.017 ($-0.022$--0.054), and the residual-minus-absolute
difference was $-0.002$ ($-0.065$--0.062). Thus, the point estimates do not resolve a
practical advantage for any score transformation. Ligand-identity permutation placed the
absolute and residual results above their nulls ($p=0.014$ and 0.024), and within-ligand
permutation of the experimental order gave $p=0.009$ and 0.012; these tests assess
association, not superiority over another representation.
...
paired interval ($-0.127$--0.066) was wide. This cohort prior is not a deployable score; it
diagnoses target and observation-process structure among these multiply assayed ligands.
...
targets. Restriction to human binding assays gave residual and cohort-prior accuracies 0.581
and 0.581 on 128 ligands; the paired interval was $-0.089$--0.092. On the 107-ligand human
binding Ki/Kd block, they were 0.579 and 0.608, with an interval of $-0.131$--0.075. Neither
residual-minus-column interval excluded zero (Supplementary Table~S5). Removing cells whose
repeated measurements spanned more than one pChEMBL unit gave residual accuracy 0.560 on
1,912 pairs.

This comparison supplies the operational boundary of the spectral analysis. Two-way
centering answers whether ligand and target main effects conceal structured residual
variation. It does not determine whether that variation orders targets correctly, and the
sparse ChEMBL observation process can make target priors stronger than molecule-specific
docking contrasts. A reverse-docking workflow should therefore report target-prior and
ligand/outcome-permutation baselines, stratify performance by experimental coverage and
choose its score transformation by external task-level validation, not by effective
dimension alone.
...
95\% Bemis--Murcko scaffold-cluster bootstrap intervals. Grey bars and crosses show the
...
with the evaluation ligand's entire scaffold cluster held out and is a diagnostic baseline
rather than a docking score.
...
Operational validation changed the interpretation again. On the expanded 137-ligand,
38-target observed-pair benchmark, absolute and two-way-residual Vina had nearly identical
score-based point estimates, and no paired score-transformation contrast was resolved. Both
exceeded identity- and outcome-permutation nulls on the inclusive support, but the ordering
reversed toward absolute Vina when ligands with only two observed targets were excluded. The
deployable external docking and ChEMBL target priors performed below the score-based point
estimates. A scaffold-held-out prior from the multiply assayed cohort was comparable or
stronger, although it is not a deployable replacement for docking and became indistinguishable
from residual Vina in the human binding sensitivity. Precisely because it reflects assay
selection, this prior demonstrates how a ranking result can depend on which targets were
measured in a broadly profiled cohort. Higher residual dimension is therefore neither
...
benchmark should additionally include target priors, ligand-identity and within-ligand
outcome-permutation nulls, and performance stratified by experimental coverage. The
operational score representation should be selected on held-out experimental target
preferences. Two-way centering can be retained as a diagnostic even when it is not the
...
co-observation has a median support of only three ligands, and its primary result includes 44
ligands observed against exactly two targets. Scaffold resampling, assay restriction,
measurement-dispersion filtering and coverage thresholds expose but cannot remove ChEMBL's
assay-selection and laboratory heterogeneity. The benchmark therefore supports an
observed-target ordinal claim, not unmeasured-target retrieval. Finally, the public package
...
an additive independent-noise null. On dense matched ChEMBL data, the transformation narrowed
the docking--experiment PR contrast. On a broader observed-pair benchmark, it did not show a
resolved target-ranking advantage over either column standardization or absolute Vina, and
the score-based ordering changed with experimental coverage. The practical consequence is
to separate diagnosis from correction: spectral analysis can identify shared ligand and
target effects, whereas a score transformation for reverse docking must earn its use against
target-prior, permutation and coverage-stratified baselines on held-out experimental data.
...
Six operational representations were compared. Absolute Vina used the original scores. A
docking target-prior baseline assigned every evaluation ligand the target means estimated
from the 12,514 Docking-44 ligands outside the evaluation cohort. Column-standardized scores
used the corresponding external target means and sample standard deviations. Two-way
residual scores additionally removed each evaluation ligand's row mean over all 44 docking
targets and the external training grand mean, then divided by external residual target
standard deviations. Missing evaluation scores were filled by target means from the same
external fit. Thus, all docking transformation and imputation parameters were fitted without
any of the 137 evaluation ligands. Relative to column standardization, this operational
residualization is a constrained target rescaling plus a ligand row effect weighted by the
inverse residual target scales, not an arbitrary learned reranker. An external experimental
prior ranked target mean pChEMBL values
estimated from the 96,744 exact-relation ChEMBL ligands outside the evaluation cohort. A
second, deliberately stringent diagnostic prior used target mean pChEMBL within the broadly
profiled evaluation cohort, fitted with the evaluation ligand's entire Bemis--Murcko scaffold
cluster held out; it measures target and observation-process structure rather than a
deployable docking rule.

Uncertainty was estimated by 5,000 ligand bootstraps and 5,000 Bemis--Murcko scaffold-
cluster bootstraps over 95 matched clusters. Paired comparisons resampled per-ligand
accuracy differences. For absolute, column-standardized and residual scores, 5,000
...
than one pChEMBL unit. Coverage-stratum intervals used the same scaffold bootstrap and are
pointwise; the nested strata are not independent. The separate dense 74-by-6 analysis retained its 100 multiple-
imputation sensitivity for spectral quantities and is reported in Supplementary Table~S3.
...
to GitHub tag \path{v1.2.0}. Upstream datasets retain their stated licenses and are identified
...
in Vina docking-score matrices: analysis code and frozen data, version 1.2.0. GitHub.
