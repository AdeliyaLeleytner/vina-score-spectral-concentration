...
\title{Spectral concentration in two large Vina docking-score matrices: two-way centering reveals correlated residual variation without a resolved target-ranking gain}
...
ligand-by-target matrix, yet shared score tendencies can obscure target-resolved variation.
We measured its target-correlation spectrum in two independently sourced AutoDock Vina
panels. Their participation-ratio (PR) effective dimensions were 1.83 and 2.27; removing
ligand and target main effects changed the estimand and increased them to 9.30 and 18.21,
while three target-wise independence nulls remained near their algebraic maxima. An observed-
pair ChEMBL benchmark compared absolute, column-standardized and residual-scaled Vina scores
without activity imputation. Their accuracies were \BroadAbsoluteAccuracy,
\BroadColumnAccuracy\ and \BroadResidualAccuracy, and the residual-minus-absolute interval
was $\ResidualAbsoluteLow$--$\ResidualAbsoluteHigh$; neither a ranking gain nor post hoc
equivalence was resolved. A human binding Ki/Kd estimand reached the same precision-limited
conclusion. Before target-specific scaling, removal of the ligand row effect was exactly
rank-invariant, so operational differences arose from scale choices rather than centering
alone. Two-way centering therefore diagnoses correlated residual variation; use of the
scaled representation for target ranking still requires external task-level validation.
...
an alternative summary of strong mean-squared inter-target correlation. PR adds no
statistical information beyond mean $r^2$ for a correlation matrix; its practical role is to
express the same dependence on a bounded scale of effective components. Mean $|r|$ gives a
familiar correlation scale, while the full spectrum and correlation matrix retain signs,
localization and the distribution of dependence among collective modes
(Table~\ref{tab:spectralmetrics}). The estimated
...
not a like-for-like missing-data correction. Although only 1.466\% of cells were missing,
3,354 ligands had at least one missing value, usually for one target (median one missing
cell; 90\% range 1--10). Missingness was highly concentrated: target 4mqs alone was absent
for 3,352 ligands. Excluded ligands were also larger than complete cases (median molecular
weight 399 versus 249 Da; median heavy-atom count 28 versus 17), with standardized mean
differences of 1.86 and 2.03. The PR shift therefore accompanies substantial chemical-
support selection rather than the nominal cell fraction alone (Supplementary Table~S2 and
the machine-readable support table). A chemical-cluster block-permutation null and
...
\input{results/main_spectral_metrics_table.tex}
...
at that level to docking raised its median PR dimension to \NoisePRMedian\ (simulation
interval \NoisePRLow--\NoisePRHigh); approximately 2.5 times that scale was required to reach the legacy
...
The residual dependence was distributed across positive and negative target pairs rather
than forming another uniformly signed axis. Mean absolute off-diagonal target correlation
fell from 0.694 to 0.246 in Docking-44 and from 0.622 to 0.155 in DOCKSTRING; the central
90\% residual-correlation ranges were $-0.450$--0.500 and $-0.328$--0.333
(Supplementary Fig.~S7). Within-family absolute correlations exceeded between-family values
by only 0.030 and 0.017, and broad-family label permutations did not resolve these small
differences ($p=0.073$ and 0.067). The leading residual modes instead remained strongly
associated with ligand chemistry: residual-PC1 scores had absolute Spearman correlations
$|\rho|=0.72$ and 0.77 with molecular weight or heavy-atom count; component signs are
arbitrary. Removing the common ligand
offset therefore leaves target-specific physicochemical slopes; the evidence does not
identify the residual modes as protein-family or biophysical specificity axes.
...
42.40 and 56.48. A third null projected independent random directions into the zero-row-sum
subspace and rescaled every simulated row to its observed residual norm. It preserved the
ligand-specific residual magnitude exactly and gave medians 42.63 and 56.62.
...
null removes the Gaussian assumption, while the random-direction null addresses
ligand-specific residual heteroskedasticity. Together they show residual dependence far
stronger than target-wise independent expectations.
...
show the fitted Gaussian additive null; crosses show the nonparametric residual-permutation
null; open triangles and lower bars show the row-norm-preserving random-direction null.
Each bar is the central 95\% of 500 realizations and is visually narrower than its marker.
All nulls remove cross-target residual correspondence; the last additionally preserves every
observed ligand-specific residual norm before column standardization.}
\label{fig:centering}
\end{figure}

\subsection{Only target-specific scaling changes target order}

We expanded the operational endpoint beyond the dense six-target block. Two complementary
estimands were declared together during revision rather than prospectively registered: an
all-endpoint analysis maximizing coverage and a human binding Ki/Kd analysis reducing assay
heterogeneity. Exact full-InChIKey
...
scaffold clusters or 121 fingerprint-based Butina clusters. All docking offsets, scales and
...
Absolute Vina reached mean per-ligand pairwise target-preference accuracy \BroadAbsoluteAccuracy
...
$\ResidualColumnLow$--\ResidualColumnHigh), and the residual-minus-absolute difference was $-0.002$
($\ResidualAbsoluteLow$--\ResidualAbsoluteHigh). Conservative unions of the corresponding 90\% intervals were
...
difference was approximately 0.054. Ordinary ligand-identity permutations placed the
absolute and residual results above their nulls ($p=0.014$ and 0.024), but that evidence did
not survive chemical de-redundancy: across 25 one-ligand-per-Murcko-cluster supports, median
permutation probabilities were 0.134 and 0.284. Within-ligand permutation of the experimental
order gave $p=0.009$ and 0.012. These tests assess score--experiment association, not
superiority of one representation, and the cluster-aware sensitivity weakens the inclusive
association claim.

Target deletion exposed an additional uncertainty axis. The residual-minus-absolute
difference ranged from $-0.030$ to 0.010 across the 38 broad-panel leave-one-target-out
analyses; for residual minus column-standardized it ranged from 0.002 to 0.029. These are
composition sensitivities conditional on the observed targets. The target panel was not
randomly sampled, so neither the ligand/scaffold bootstrap nor the target jackknife licenses
superpopulation inference to unmeasured targets.
...
excluded, reached 0.592 (\CohortPriorLow--\CohortPriorHigh). It exceeded residual Vina by 0.028 on average, but the
...
The assay-restricted estimand reached the same precision-limited conclusion. On 107 ligands
and 30 targets with human binding Ki/Kd measurements, absolute, column-standardized and
residual accuracies were \KiKdAbsoluteAccuracy, \KiKdColumnAccuracy\ and
\KiKdResidualAccuracy\ over 2,224 pairs; residual minus absolute had a cluster-interval union
of $-0.051$--0.085. Its leave-one-target-out difference ranged from $-0.027$ to 0.031. The
positive residual point shift was therefore unresolved and target-composition sensitive.

Holding the broad 137-by-38 support fixed but comparing only cells with the same endpoint
type produced 4,330 endpoint-specific instances across 132 evaluable ligands. Residual
accuracy was 0.563 for Ki, 0.524 for Kd, 0.516 for IC50 and 0.398 for EC50; the Kd and EC50
estimates used only seven and 18 evaluable ligands. Because a target pair could occur under
more than one endpoint, a duplicate-collapsed sensitivity averaged each ligand--target-pair
once after endpoint-specific scoring. It retained 2,460 unique instances and gave absolute,
column-standardized and residual accuracies \DuplicateCollapsedAbsoluteAccuracy,
\DuplicateCollapsedColumnAccuracy\ and \DuplicateCollapsedResidualAccuracy, again without a
resolved residual-minus-absolute difference. Removing cells whose repeated measurements
spanned more than one pChEMBL unit gave residual accuracy 0.560 on 1,912 pairs
(Supplementary Table~S5 and the accompanying machine-readable tables).

This comparison supplies the operational boundary of the spectral analysis. Two-way
centering reveals whether ligand and target main effects conceal correlated residual
...
labels give the ordinary one-sided probability and the median probability across 25
one-ligand-per-Murcko-cluster supports. The cohort experimental prior is fitted
...
and both empirical-marginal and row-norm-preserving nulls placed target-wise independent
residuals near 43 and 57 dimensions. The empirical residuals therefore contain more
directions than the shared-score surface while remaining strongly correlated. The result is
a reduction in spectral concentration, not its disappearance.
...
protocol rather than present centering, PCA or PR as a new algorithm. PR is exactly a
monotone re-expression of mean squared target correlation; its advantage is interpretive
scale, while the correlation matrix and full spectrum remain necessary for signs,
localization and mode shape.
...
The residual covariance also has a concrete, if incomplete, interpretation. Its leading
modes remain associated with molecular size, consistent with target-specific size slopes
that survive removal of a common ligand offset. Broad protein-family labels explain at most
a weak part of the residual correlation pattern on these panels. We therefore use
``correlated residual variation'' rather than equating it with biological specificity;
sequence- or pocket-level attribution remains open.
...
absolute scores. Ordinary identity and outcome permutations placed both above their nulls
on the inclusive support, but one-ligand-per-scaffold permutations weakened the identity
evidence, and the ordering shifted toward absolute Vina when ligands with only two observed
...
column-standardized surface and the two-way-centered residual estimands; (3) report mean
$r^2$, mean $|r|$, PR and entropy effective dimensions, PC1 fraction, correlation
distributions, cumulative-component counts and the algebraic
...
empirical residual-permutation and row-norm-preserving nulls; (7) for ranking applications,
test invariances and
...
produces a precise score-transformation contrast. Target deletion further shows that the
paired differences depend on the fixed target composition. Scaffold resampling,
...
higher-dimensional correlated residual variation, but the residuals remain far more
concentrated than Gaussian, empirical-marginal or row-norm-preserving target-wise independent
nulls. On dense matched ChEMBL data,
...
strongly. We additionally reported mean and median absolute off-diagonal correlation, the
signed 5th--95th percentile range and the fraction of positive correlations. PR and mean
squared correlation are algebraically equivalent; the other summaries and the full matrix
retain information discarded by that scalar identity. Effective ranks are continuous
summaries of variance concentration, not algebraic or nonlinear manifold ranks.
...
The row-norm-preserving null started from the same observed residual support. Independent
Gaussian vectors were projected into the $P-1$ dimensional target-zero-sum subspace and each
simulated row was rescaled to the Euclidean norm of its corresponding observed row. This
preserves every ligand-specific residual magnitude and the zero-row-sum constraint exactly
before column standardization, while removing target alignment and target-specific
marginals. We generated 500 realizations for each large matrix.

For residual characterization, targets were ordered by average-linkage clustering of
$1-r_{jk}$. We compared mean absolute within-family and between-family correlations and
permuted the fixed broad family labels 5,000 times. The three leading residual target modes
were obtained from the same correlation matrix. Their ligand scores were correlated by
Spearman's coefficient with molecular weight, heavy-atom count, cLogP, topological polar
surface area, rotatable-bond count and ring count. These descriptive analyses used fixed
seeded supports of 12,000 and 15,000 ligands and do not test sequence or pocket mechanisms.
...
changes the chemical support and is not a like-for-like imputation test. We therefore
compared complete and excluded ligands using six RDKit descriptors, standardized mean
differences and Butina-cluster coverage, and tabulated missing cells per row and target.
Censoring
...
statistic was the mean of per-ligand pairwise accuracies, giving each ligand equal weight; a
predicted score tie received half credit and pair-weighted accuracy was secondary. The broad
all-endpoint and human binding Ki/Kd estimands were designated together during revision and
are reported without selecting between them by outcome; this designation was not preregistered.
...
over 121 matched clusters. Paired comparisons resampled per-ligand accuracy differences.
...
One-sided probabilities used the finite-permutation correction. A chemical-dependence
sensitivity generated 25 supports by selecting one ligand per Bemis--Murcko cluster and ran
200 identity permutations within every support. Leave-one-target-out analysis recomputed
paired representation differences after deleting each observed target; its range and
jackknife approximation describe fixed-panel composition sensitivity, not inference to a
random target population. Sensitivities used minimum
...
the same target pair in more than one endpoint stratum. We therefore added a duplicate-
collapsed estimator that first averaged correctness for each unique ligand and unordered
target pair across eligible endpoint strata, then averaged pairs within ligand and ligands
equally. Coverage-stratum intervals used the
...
to GitHub tag \path{v1.4.0}. Upstream datasets retain their stated licenses and are identified
...
in Vina docking-score matrices: analysis code and frozen data, version 1.4.0. GitHub.
